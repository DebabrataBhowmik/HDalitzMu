...
void draw_var(ROOT::RDF::RNode df_mc, ROOT::RDF::RNode df_data, string varName, string sel, vector<double> bins, string legName){
    // preprocess mc dataframe
    auto nf_mc = df_mc;
    if (!nf_mc.HasColumn(varName))
        nf_mc = nf_mc.Define(varName, sel);
    ...
    auto h_mc = nf_mc.Histo1D({Form("h_%s_mc", varName.c_str()), "", (int)bins[0], bins[1], bins[2]}, varName, "weight");
    // h_mc->Scale(100);
    ...
    h_data->GetYaxis()->SetRangeUser(0, h_data->GetBinContent(h_data->GetMaximumBin()) * 1.5);
    ...
    h_mc->SetFillColor(TColor::GetColor("#F8CE68"));
    h_mc->SetLineColor(TColor::GetColor("#F8CE68"));
    h_mc->Draw("hist same");
    h_data->Draw("EP same");
    canv->RedrawAxis();
    ...
    auto leg = new TLegend(0.69, 0.79, 0.9, 0.88);
    leg->SetTextFont(42);
    leg->SetTextSize(0.035);
    leg->SetFillColor(0);
    leg->SetLineColor(0);
    leg->AddEntry(h_data.GetPtr(), "data", "LE1P");
    leg->AddEntry(h_mc.GetPtr(), "Background", "f");
    leg->Draw("same");
...
void drawKinematics(){
    ...
    for (auto it = var_sel.begin(); it != var_sel.end(); it++){
        ...
        draw_var(df_mc, df_data, varName, sel, bins, legName);
    }
}